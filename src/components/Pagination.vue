...
<template>
    <ul class="flex">
        <li class="w-8 h-8 rounded-md flex justify-center items-center mr-1 text-lg "
            :class="{ 'bg-[#EEEEEF] hover:bg-[#E9E9EA] cursor-pointer': currentPage != 1 }" @click.prevent="previousPage">
            <p>&lt;</p>
        </li>
        <li class="w-8 h-8 bg-[#EEEEEF] hover:bg-[#E9E9EA] cursor-pointer rounded-md flex justify-center items-center mr-1 text-sm"
            :class="{ 'bg-blue-100 hover:bg-blue-50': currentPage == 1 }" @click.prevent="changePage(1)">
            <a href="#">1</a>
        </li>
        <li v-if="showDotsBefore"><span>...</span></li>
        <li class="w-8 h-8 bg-[#EEEEEF] hover:bg-[#E9E9EA] cursor-pointer rounded-md flex justify-center items-center mr-1 text-sm"
            :class="{ 'bg-blue-100 hover:bg-blue-50': currentPage == page }" v-for="(page, index) in pages" :key="index"
            @click.prevent="changePage(page)">
            <a href="#">{{ page }}</a>
        </li>
        <li v-if="showDotsAfter && totalPages"><span>...</span></li>
        <li class="w-8 h-8 bg-[#EEEEEF] hover:bg-[#E9E9EA] cursor-pointer rounded-md flex justify-center items-center mr-1 text-sm"
            :class="{ 'bg-blue-100 hover:bg-blue-50': currentPage == totalPages }" v-if="totalPages > maxVisiblePages"
            @click.prevent="changePage(totalPages)">
            <a href="#">{{ totalPages }}</a>
        </li>
        <li class="w-8 h-8 rounded-md flex justify-center items-center text-lg cursor-pointer"
